/**
 * Sets the browser_action (toolbar) icon per-tab to reflect
 * the current container's color. Solid circle = saved, ring = ephemeral.
 */
const PageActionIndicator = (() => {
  ...
  function getIconData(colorName, saved) {
    ...
    const data = {
      16: generateIcon(hex, 16, saved),
      32: generateIcon(hex, 32, saved),
    };
  ...
  async function _doUpdate(tabId) {
    try {
      ...
      let color = 'toolbar';
      let saved = false;
      let label = 'No container';

      if (tab.cookieStoreId !== 'firefox-default') {
        try {
          const container = await browser.contextualIdentities.get(tab.cookieStoreId);
          color = container.color;
          saved = !ContainerManager.isEphemeral(tab.cookieStoreId);
          label = saved ? container.name : `${container.name} (ephemeral)`;
        } catch {
          return;
        }
      }
